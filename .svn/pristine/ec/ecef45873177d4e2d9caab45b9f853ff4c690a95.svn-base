	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="background-color: #f2f2f2;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="primary" @click="handleAdd">新增美剧</el-button>
				</el-form-item>
			</el-form>
		</el-col>
            ...
            <el-option v-for="item in moviesOptions" :key="item.id" :value="item.id" :label="item.title">
				...
				<el-form-item label="视频封面">
					<el-upload
					  ...
					  action="https://lao47.xin/api/img_upload"
...
<script>
	...
	import { getRestrictedListPage, submitForm1, romveRestrictedList, getNewmovies, getMoviesMessage, updateResources } from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			moviesMessClick(){
				getMoviesMessage({'id':this.addForm._id}).then((res)=>{
	      ...
	      if (query !== '') {
			...
			handleDel: function (index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					//NProgress.start();
					let para = { id: row.id };
					romveRestrictedList(para).then((res) => {
						this.listLoading = false;
						//NProgress.done();
						this.$message({
							message: res.message,
